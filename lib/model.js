...
Model.prototype.$__handleSave = function(options, callback) {
  ...
  if (this.isNew) {
    ...
    const timestampOption = this.schema.options.timestamps;
    const useMongoTimestamp = (typeof this.schema.options.timestamps === 'object')
      && !!this.schema.options.timestamps.useMongoTimestamp;

    if (useMongoTimestamp) {
      const hasCustomKey = (typeof timestampOption === 'object') &&
        (typeof timestampOption.updatedAt === 'string');

      const updateTimestampKey = hasCustomKey
        ? timestampOption.updatedAt
        : 'updatedAt';

      if (Array.isArray(delta) && delta.length >= 2) {
        delta[1]['$currentDate'] = { [updateTimestampKey]: true };
      }
    }
